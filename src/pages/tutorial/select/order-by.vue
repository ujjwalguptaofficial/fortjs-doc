<template><Layout title='Order By' description='learn how to use order data in indexeddb' keywords='order by, sort by, select, indexeddb, jsstore' contentSrc='/home/warrior/projects/opensource/doc-template/content/tutorial/select/order-by.md'><p>Order By is used to sort the data in ascending or descending order, based on any column.</p>
<h4 id="sql">Sql</h4>
<pre><code>Select * from Table_Name Order by column_name sort_type;
</code></pre>
...
<li><p>idbSorting: boolean // whether to do sorting by indexeddb or by jsstore, default - true</p></li>
</ul>
...
<h2 id="orderbymultiplecolumn">Order by multiple column</h2>
<p><br>For ordering by multiple column - you need to provide all order object value in an array</p>
<pre><code>var results = await connection.select({
    from: "Table_Name",
    order: [{
        by: column_name1,
